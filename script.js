        ...
        <tbody>
          ${rows.map(row => `
            <tr>
              ...
              <td>${row.url
                ? `<a class="open-link" href="${encodeURI(row.url)}" target="_blank" rel="noopener">Abrir ↗</a>`
...
Promise.all([
  ...
  fetch("data/materias-archivos.json").then(r => r.json()).catch(() => []), // catálogo autogenerado, puede no existir aún
  fetch("data/recursos.json").then(r => r.json()),
  fetch("data/historietas.json").then(r => r.json()),
]).then(([materiasManual, materiasAuto, recursos, historietas]) => {
  renderFilterableTable({
    mountId: "materiasTable",
    filtersId: "materiasFilters",
    data: [...materiasManual, ...materiasAuto],
